...
Nothing regressed in the system. The safeguard simply no longer names the
category. **This is the cleanest evidence in the whole evaluation that safeguard
wording — not just which categories are nominally covered — determines what
passes.** In deployment the exposure is smaller than it looks, because Pip's
chat contract declines such requests before the image route is reached, but that
path is measured separately and is not what these numbers describe.
